...
func refresh() -> void:
	...
	if p.pending_dream:
		# ผ่านเกณฑ์แล้ว รอแค่ทอยความฝัน — หน้าจอทอยจะเด้งขึ้นมาทับแผงนี้อยู่แล้ว
		# บรรทัดนี้จึงเป็นตัวกันพลาด เผื่อวันไหนหน้าทอยไม่ได้เด้ง ผู้เล่นจะยังรู้ว่าเกิดอะไรขึ้น
		_body.add_child(_line(
			"ผ่านเกณฑ์ด่าน 1 แล้ว! รอทอยเต๋าสุ่มความฝันเพื่อเข้าด่าน 2 (GDD บทที่ 9)", GOLD))
	else:
