...
func _init() -> void:
	...
	# แค่เช็กจำนวนไม่พอ — ขนาดเป็น 4 อยู่แล้วตั้งแต่ก่อนเรียก ถ้า `fill_bots` เผลอเอาบอทไปแทน
	# คนจริงสักที่นั่ง (ขนาดยังเท่าเดิม) เทสต์ข้างบนจะยังผ่านอยู่ดี ต้องเช็กว่าไม่มีที่นั่งไหน
	# เป็นบอทเลยสักที่ ถึงจะพิสูจน์ว่า "เต็มแล้วไม่แตะที่นั่งคนจริง" จริงๆ
	var any_bot := false
	for seat in full:
		if bool(seat.is_ai): any_bot = true
	_eq("โต๊ะที่คนจริงเต็มแล้วต้องไม่มีที่นั่งไหนกลายเป็นบอท", any_bot, false)
