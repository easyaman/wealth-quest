...
**ม่านถ่ายภาพด้วย `WQ_SHOT` ไม่ได้** — มันโผล่หลังตั้งที่นั่งครบ ซึ่งต้องกดผ่านหน้าทอยเต๋าจริงสองรอบก่อน
และ `WQ_ROLL` ก็ช่วยไม่ได้เพราะมันกระโดดข้ามหน้าอินโทรไปหน้าเลือกอาชีพเลย (`skip_to_jobs()` ตั้ง
`_intro.visible = false`) ต้องเปิดเกมแล้วดูด้วยตา:

```bash
cd godot && WQ_HUMANS=2 godot --path .
```

1. ทอยเต๋าและเลือกอาชีพให้ผู้เล่น 1 → ทอยและเลือกให้ผู้เล่น 2
2. ตั้งครบปุ๊บ **ต้องเห็นม่านทึบขึ้นชื่อผู้เล่น 1 ทันที** ไม่ใช่เห็นจอเกม
3. กดพร้อม → เห็นจอของผู้เล่น 1 → กด "จบตา ▸" → **เห็นม่านทึบขึ้นชื่อผู้เล่น 2**
...
**และแก้บรรทัดที่กลายเป็นคำโกหกหลังงานนี้** — `godot` เปล่าๆ ไม่ได้เริ่มที่หน้าเลือกอาชีพอีกแล้ว
มันเริ่มที่หน้าเมนูเลือกจำนวนคน · ส่วน `WQ_ROLL` ต้องบอกให้ชัดว่ามันมีผลตอนเข้าหน้าทอยเต๋า
(หลังเลือกจำนวนคนแล้ว) ไม่ใช่ตั้งแต่เปิดเกม — ไล่แก้ให้ครบทั้ง `godot/README.md`
และ `godot/CLAUDE.md` (ใน `CLAUDE.md` คือบรรทัดราวๆ 219 กับ 222)
