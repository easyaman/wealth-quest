...
func _start_match() -> void:
	...
	#
	# ไม่ต้องเรียก `_maybe_pass_screen()` ซ้ำตรงนี้ — `_adopt_match()` ที่เรียกไปตอนต้นฟังก์ชัน
	# ยกม่านให้แล้วก่อน `_refresh()` ตัวแรกของมันด้วยซ้ำ (เร็วกว่าจุดนี้อีก) ตราบใดที่ไม่มีอะไร
	# ระหว่างทางไปเปลี่ยนตัวผู้เล่นปัจจุบันหรือสั่งปิดม่าน (ไม่มี — บล็อกด้านบนแค่ปรับสถานที่/สอน)
	# ม่านที่ขึ้นไปแล้วยังอยู่ครบถึงบล็อก WQ_DREAM ด้านล่างแน่นอน
...
func _adopt_match(new_match: WQMatch) -> void:
	...
	# ตำแหน่งนี้กำหนดไว้แบบเดียวกับ `_end_turn()`: ม่านต้องขึ้นก่อน `_refresh()` เสมอ เพราะ
	# `_refresh()` เรียก `_check_pending_dream()` ซึ่งยืนรอก็ต่อเมื่อเห็น `pass_screen` อยู่แล้ว
	# เท่านั้น — ทั้งเริ่มเกมใหม่และโหลดไฟล์เซฟมาที่ฟังก์ชันนี้ ถ้าสลับลำดับ คนที่กดโหลด
	# กลางโต๊ะจะเจอการ์ดความฝันของอีกคนโผล่ใส่หน้าตัวเองก่อนม่านจะทันขึ้น
	_maybe_pass_screen()
	_refresh()


func _on_month_ended(_mo: int) -> void:
	# `end_month()` (core/match.gd) ยิงสัญญาณนี้ **ก่อน** ตั้ง `turn = 0` และก่อน `run_ai_turns()`
	# เดินหา "คนจริงคนถัดไป" — ถ้าเขียนไฟล์ตรงนี้ตรงๆ จะได้ `turn == players.size()` ติดอยู่ใน
	# ไฟล์ทุกครั้ง (นอกช่วง array) แล้ว `get_current()` คืน null ตลอดไป: โหลดกลับมาบอร์ดว่าง
	# ไม่มีม่านกั้น (รูรั่วความเป็นส่วนตัวของ hot-seat) และกด "จบตา" ครั้งแรกจะ crash ที่
	# `get_current().hours` — ต้องเลื่อนการเขียนไปหลังเฟรมนี้คลี่ตัวจบด้วย `call_deferred`
	# ณ จุดนั้น `run_ai_turns()` เดินจบรอบแล้ว `turn` ชี้ไปที่คนจริงคนถัดไปแน่นอน (ยืนยันด้วย
	# print ชั่วคราวแล้วก่อนแก้ — ดูรายงาน FIX 2) `_refresh()` ยังอยู่ตรงนี้เหมือนเดิม เพราะ
	# หน้าจอต้องอัปเดตทันทีที่เดือนขึ้น ไม่ต้องรอรอบเดียวกับไฟล์เซฟ
	call_deferred("_write_autosave")
	_refresh()


func _write_autosave() -> void:
	# กันไว้เฉยๆ เผื่อ `m` ถูกแทนที่ก่อนคิวนี้จะได้รัน (โหลดไฟล์ใหม่กลางอากาศ) — ไม่มีทางเกิด
	# ในเส้นทางเล่นปกติเพราะเฟรมเดียวกันไม่มีจังหวะให้ผู้เล่นกดอะไรคั่นได้ แต่เขียนทับ `m` ผิดตัว
	# ยังแย่กว่าไม่เขียนเลย
	if m == null: return
...
func _on_load_slot(slot: String) -> void:
	...
	if dream_screen != null:
		...
		dream_screen = null
	# ม่านของโต๊ะที่กำลังจะถูกแทนที่ต้องหายไปด้วย ไม่งั้นม่านเก่า (ผูกกับคนละแมตช์)
	# ค้างทับจอ ในขณะที่ `_adopt_match()` ด้านล่างกำลังจะยกม่านใหม่ของแมตช์ที่โหลดมาซ้อนอีกชั้น
	if pass_screen != null:
		_close_screen(pass_screen)
		pass_screen = null
	WQAudio.ui("load")
	# `_adopt_match()` เป็นคนยกม่านให้เอง (ก่อน `_refresh()` ภายในของมัน) — คนที่กดโหลดอาจไม่ใช่
	# คนที่ถึงตา จึงต้องมีม่านกั้นเหมือนตอนจบตา ไม่ต้องเรียก `_maybe_pass_screen()` ซ้ำอีกทีที่นี่
	_adopt_match(loaded)
...
func _end_turn() -> void:
	...
	# ม่านส่งเครื่อง (`pass_screen`) ก็ต้องกันด้วยเหตุผลเดียวกัน — ม่านบังจอไว้ให้คนที่ "ยังไม่พร้อม"
	# เห็นเกม ถ้าจบตาทะลุม่านได้ ตาของเขาจะถูกกินไปเงียบๆ แล้วทุกวิดเจ็ตจะผูกกับผู้เล่นคนถัดไปทันที
	# พอเจ้าตัวกดม่านออก จะเห็น (และกดของ) มือของคนอื่นได้เต็มๆ ซึ่งคือรูรั่วที่ม่านมีไว้ปิดโดยตรง
...
##
## ตั้งใจนับ "ที่นั่งคนจริงทั้งหมด" ไม่ใช่ "คนจริงที่ยังเล่นอยู่" — โต๊ะสองคนที่คนหนึ่งล้มละลาย
## ไปแล้วจะยังเห็นม่านก่อนทุกตาของคนที่เหลือ ทั้งที่ตอนนี้เหลือแค่คนเดียวเล่นจริงๆ นี่คือพฤติกรรม
## เดียวกับต้นแบบเว็บ (`ui.html:438` ใช้ `setupList.length>1` ไม่ใช่จำนวนที่ยังเล่นอยู่) และการอ่าน
## จำนวนจากแมตช์ตรงๆ (ไม่แคชไว้) คือสิ่งที่ทำให้ม่านรอดจากเซฟ/โหลดได้โดยไม่ต้องเก็บสถานะเพิ่ม
## — **อย่าแก้ให้กรองบอทล้มละลาย/จบเกมออก** แม้จะดูสมเหตุสมผลกว่าในเคสเดี่ยว
...
func _maybe_pass_screen() -> void:
	...
	# ต้องแย่ง focus มาจากปุ่มของกระดานเดิม (เช่นปุ่ม "จบตา" ที่เพิ่งกดไป) ไม่งั้นปุ่มนั้นยัง
	# focus ค้างอยู่ใต้ม่าน แล้ว SPACE ครั้งถัดไปจะวิ่งเป็น `ui_accept` ตรงไปหาปุ่มที่ focus อยู่
	# (Control รับ ui_accept ก่อน `_unhandled_input` จะได้เห็นเลยด้วยซ้ำ) — โฟกัสปุ่มของม่านเอง
	# ทำให้ SPACE แปลว่า "พร้อมแล้ว เริ่มตาของฉัน" เท่านั้น ไม่มีทางหลุดไปโดนปุ่มหลังม่านอีก
	pass_screen._btn.grab_focus()
