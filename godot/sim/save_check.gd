...
func _init() -> void:
	...
	# หัวไฟล์โชว์ได้แค่คนเดียว (ชื่อ/อาชีพ/สุทธิ) — ต้องเป็นคนจริง "คนแรก" ที่นั่งไว้เสมอ
	# ไม่ใช่คนสุดท้ายที่วนเจอ ไม่งั้นรายการเซฟจะโชว์คนละคนกันทุกครั้งที่ลำดับผู้เล่นเปลี่ยน
	_eq("หัวไฟล์เลือกคนจริงคนแรกไว้แสดง", String(WQSave.meta_of(hs).get("name", "")), "ผู้เล่น 1")
